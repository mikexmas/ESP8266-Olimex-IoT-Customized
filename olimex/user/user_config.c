	...
	if (method == POST && data != NULL && data_len != 0) {
		...
		while ((json_type = jsonparse_next(&parser)) != 0) {
			if (json_type == JSON_TYPE_PAIR_NAME) {
				...
				// Added by MihaB
				else if (jsonparse_strcmp_value(&parser, "ConfigLoad") == 0) {
					debug("CONFIG: Load CONFIG settings\n");
					user_config_load();
				} else if (jsonparse_strcmp_value(&parser, "ConfigRestore") == 0) {
					debug("CONFIG: Load CONFIG restored\n");
					user_config_restore_defaults();
					user_config_load();
				}
			}
